...
import { formatChoicesForSettings, formatSkillsForDropdown, formatAbilitiesForDropdown } from "./system-data.js";
...
Hooks.once("init", () => {
  ...
  game.settings.register("5e-training", "defaultAbility", {
    ...
    config: true,
    type: String,
    choices: formatChoicesForSettings(),
    default: "int",
  });
...
async function addTrainingTab(app, html, data) {
  ...
  if (showTrainingTab){
    ...
    const ABILITIES = formatAbilitiesForDropdown();
    const SKILLS = formatSkillsForDropdown();
